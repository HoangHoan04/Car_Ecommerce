  ...
  let modalHtml = `
    ...
    <div class="modal-footer">
        ...
        <div class="modal-footer-control">
            <button class="button-order-now" data-product="${
              infoProduct.id
            }">Đặt xe ngay</button>
...
function openSearchMb() {
  ...
  document.querySelector(".header-container-user-item.close").style.display =
    "block";
...
signupButton.addEventListener("click", (event) => {
  ...
  } else if (/\d/.test(fullNameUser)) {
    document.querySelector(".form-message-name").innerHTML =
      "Họ tên không được chứa ký tự số";
    document.getElementById("fullname").focus();
  } else if (
    /[^a-zA-Z\sàáảãạâấầẩẫậăắằẳẵặèéẻẽẹêềếểễệìíỉĩịòóỏõọôồốổỗộơờớởỡợùúủũụưừứửữựỳýỷỹỵđĐ]+/.test(
      fullNameUser
    )
  ) {
    document.querySelector(".form-message-name").innerHTML =
  ...
  } else if (!/^(0|F0)[0-9]{9,10}$/.test(phoneUser)) {
    document.querySelector(".form-message-phone").innerHTML =
      "Số điện thoại phải bắt đầu bằng '0' hoặc 'F0' và có 10-11 chữ số.";
    document.getElementById("phone").value = "";
  } else {
    document.querySelector(".form-message-phone").innerHTML = "";
  }
  ...
  } else if (idUser.length <= 6) {
    document.querySelector(".form-message-username").innerHTML =
      "Tên đăng nhập phải trên 6 ký tự";
  } else {
    document.querySelector(".form-message-username").innerHTML = "";
  }
...
loginButton.addEventListener("click", (event) => {
  ...
  if (userNamelog.length == 0) {
    document.querySelector(".form-message.userNamelog").innerHTML =
      "Vui lòng nhập vào tên đăng nhập";
  } else if (userNamelog.length <= 6) {
    document.querySelector(".form-messsage.userNamelog").innerHTML =
      "Tên đăng nhập phải có nhiều hơn 6 ký tự";
  } else {
    document.querySelector(".form-message.userNamelog").innerHTML = "";
  }
    ...
    if (vitri == -1) {
      ...
      if (accounts[vitri].status == 0) {
        ...
        toast({
          ...
          message: "Chào mừng bạn đến với SaiThanh Showroom",
          type: "success",
          duration: 2000,
        });
        closeModal();
                ...
                <div class="order-history">
                    <div class="order-history-left">
                        ...
                        <div class="order-history-info">
                            <h4>${infosp.title}</h4>
                            <p>- ${infosp.style} -</p>
                            <p class="order-history-quantity">Số lượng: ${
                              sp.soluong
                            }</p>
                        </div>
                    </div>
...
function searchProducts(mode) {
  ...
  let valueStyle = document.getElementById(
    "advanced-search-Style-select"
  ).value;
  let valueBrand = document.getElementById(
    "advanced-search-Brand-select"
  ).value;
